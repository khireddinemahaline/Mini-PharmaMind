...
const isValidTargetSearchArgs = (args: any): args is { query: string; size?: number; format?: string } => {
  return (
    ...
    args.query.length > 0 &&
    (args.size === undefined || (typeof args.size === 'number' && args.size > 0 && args.size <= 8)) &&
...
class OpenTargetsServer {
      ...
      tools: [
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              size: { type: 'number', description: 'Keep this compact for context efficiency; default 4, max 8', minimum: 1, maximum: 8 },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              size: { type: 'number', description: 'Default 8, max 8 for compact evidence review', minimum: 1, maximum: 8 },
            },
            required: [],
          },
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              diseaseId: { type: 'string', description: 'Disease EFO ID' },
              minScore: { type: 'number', description: 'Minimum association score (0-1)', minimum: 0, maximum: 1 },
              size: { type: 'number', description: 'Default 8, max 8 for compact candidate lists', minimum: 1, maximum: 8 },
            },
            required: ['diseaseId'],
          },
        },
  ...
  private async handleSearchTargets(args: any) {
    ...
    try {
      ...
      const limitedHits = hits.slice(0, Math.min(args.size ?? DEFAULT_SEARCH_LIMIT, DEFAULT_SEARCH_LIMIT));
      const result = {
        ...response.data,
        data: {
          search: {
      ...
      if (args.targetId && !args.diseaseId) {
        ...
        const response = await this.graphqlClient.post('', {
          ...
          variables: {
            ...
            size: Math.min(args.size ?? DEFAULT_ASSOCIATION_LIMIT, DEFAULT_ASSOCIATION_LIMIT)
          }
        });
  ...
  private async handleGetDiseaseTargetsSummary(args: any) {
    ...
    try {
      ...
      const summary = {
        ...
        topTargets: associations?.rows?.slice(0, DEFAULT_ASSOCIATION_LIMIT).map((assoc: any) => ({
