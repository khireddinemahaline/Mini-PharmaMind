...
function isValidSize(value: unknown, max: number): value is number {
  return (
    typeof value === 'number' &&
...
function isValidSearchArgs(args: unknown): args is SearchArgs {
  ...
  if (args.size !== undefined && !isValidSize(args.size, MAX_SEARCH_LIMIT)) {
    return false;
  }
...
function isValidAssociationArgs(args: unknown): args is AssociationArgs {
  if (!isObject(args)) return false;

  const hasTarget = args.targetId !== undefined;
  const hasDisease = args.diseaseId !== undefined;

  if (!hasTarget && !hasDisease) {
    return false;
  }

  if (args.targetId !== undefined && !isNonEmptyString(args.targetId)) {
    return false;
  }

  if (args.diseaseId !== undefined && !isNonEmptyString(args.diseaseId)) {
    return false;
  }

  if (args.minScore !== undefined && !isValidScore(args.minScore)) {
    return false;
  }
  ...
  if (args.pageIndex !== undefined && !isValidPageIndex(args.pageIndex)) {
    return false;
  }
...
function isValidDiseaseTargetsArgs(args: unknown): args is DiseaseTargetsArgs {
  if (!isObject(args)) return false;
...
function isValidDiseaseDrugsArgs(args: unknown): args is DiseaseDrugsArgs {
  if (!isObject(args)) return false;
  ...
  if (args.size !== undefined && !isValidSize(args.size, MAX_DRUG_LIMIT)) {
    return false;
  }
...
function isValidTargetDrugsArgs(args: unknown): args is TargetDrugsArgs {
  if (!isObject(args)) return false;
...
function isValidIdArgs(args: unknown): args is IdArgs {
  return isObject(args) && isNonEmptyString(args.id);
}
...
function getSize(
  ...
  return Math.min(requested ?? defaultValue, maxValue);
}

function getPageIndex(requested: number | undefined): number {
  return requested ?? 0;
}
...
class OpenTargetsServer {
  ...
  constructor() {
    ...
    process.on('SIGINT', async () => {
      console.error('Shutting down Open Targets MCP server...');
  ...
  private errorResponse(message: string, error: unknown) {
    return {
      content: [
        {
          type: 'text',
  ...
  private setupResourceHandlers() {
      ...
      resourceTemplates: [
        {
          ...
          description: 'Compact target information for an Ensembl gene ID',
        },
        {
    ...
    this.server.setRequestHandler(ReadResourceRequestSchema, async (request: any) => {
      ...
      const targetMatch = uri.match(/^opentargets:\/\/target\/([^/]+)$/);

      if (targetMatch) {
        const targetId = decodeURIComponent(targetMatch[1]);
        const result = await this.fetchTargetDetails(targetId);
      ...
      const diseaseMatch = uri.match(/^opentargets:\/\/disease\/([^/]+)$/);

      if (diseaseMatch) {
        const diseaseId = decodeURIComponent(diseaseMatch[1]);
        const result = await this.fetchDiseaseDetails(diseaseId);
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
         ...
         * ------------------------- */
        {
          ...
          inputSchema: {
            ...
            properties: {
              query: {
                type: 'string',
                description: 'Gene symbol, target name, or target description',
              },
              size: {
                type: 'integer',
                minimum: 1,
                maximum: MAX_SEARCH_LIMIT,
                description: `Default ${DEFAULT_SEARCH_LIMIT}, maximum ${MAX_SEARCH_LIMIT}`,
              },
            },
            required: ['query'],
          },
         ...
         * ------------------------- */
        {
          ...
          inputSchema: {
            ...
            properties: {
              query: {
                type: 'string',
                description: 'Disease name, synonym, or phenotype',
              },
              size: {
                type: 'integer',
                minimum: 1,
          ...
          inputSchema: {
            ...
            properties: {
              ...
              minScore: {
                ...
                minimum: 0,
                maximum: 1,
                description: 'Optional minimum association score',
              },
              size: {
                type: 'integer',
                minimum: 1,
                maximum: MAX_ASSOCIATION_LIMIT,
              },
              pageIndex: {
                type: 'integer',
                minimum: 0,
         ...
         * ------------------------- */
        {
          ...
          inputSchema: {
            ...
            properties: {
              diseaseId: {
                type: 'string',
                description: 'Open Targets disease identifier returned by search_diseases',
              },
              minScore: {
                type: 'number',
                minimum: 0,
         ...
         * ------------------------- */
        {
          ...
          description: 'Get compact target details.',
          inputSchema: {
            type: 'object',
            properties: {
              id: { type: 'string' },
...

        {
          ...
          description: 'Get compact disease details.',
          inputSchema: {
            type: 'object',
            properties: {
              id: { type: 'string' },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
      ...
      switch (name) {
        ...
        case 'get_disease_drugs':
          return this.handleGetDiseaseDrugs(args);

        case 'get_target_drugs':
          return this.handleGetTargetDrugs(args);
  ...
  private async handleSearchTargets(args: unknown) {
    if (!isValidSearchArgs(args)) {
      throw new McpError(ErrorCode.InvalidParams, 'Invalid target search arguments');
    }

    try {
      const size = getSize(args.size, DEFAULT_SEARCH_LIMIT, MAX_SEARCH_LIMIT);

      const query = `
        query SearchTargets($queryString: String!, $pageSize: Int!) {
          search(
      ...
      return this.textResponse({
        ...
        returned: data.search.hits.length,
        results: data.search.hits,
      });
    } catch (error) {
      return this.errorResponse('Error searching targets', error);
    }
  }
  ...
  private async handleSearchDiseases(args: unknown) {
    if (!isValidSearchArgs(args)) {
    ...
    try {
      ...
      const query = `
        query SearchDiseases($queryString: String!, $pageSize: Int!) {
          search(
            queryString: $queryString
            entityNames: ["disease"]
            page: { index: 0, size: $pageSize }
      ...
      return this.errorResponse('Error searching diseases', error);
    }
  }
  ...
  private async handleGetTargetDiseaseAssociations(args: unknown) {
    if (!isValidAssociationArgs(args)) {
      throw new McpError(ErrorCode.InvalidParams, 'Invalid association arguments');
    }

    try {
      const size = getSize(args.size, DEFAULT_ASSOCIATION_LIMIT, MAX_ASSOCIATION_LIMIT);
      const pageIndex = getPageIndex(args.pageIndex);
      ...
      if (args.targetId && !args.diseaseId) {
        const query = `
          query GetTargetAssociations($ensemblId: String!, $pageIndex: Int!, $pageSize: Int!) {
        ...
        // Fix: capture minScore in a local const so TS narrowing
        // survives inside the filter closure (TS18048).
        if (args.minScore !== undefined) {
          const minScore = args.minScore;
          rows = rows.filter((row: any) => row.score >= minScore);
        }
        ...
        return this.textResponse({
          ...
          totalAssociations: target.associatedDiseases?.count ?? 0,
          pageIndex,
          requestedSize: size,
          returned: rows.length,
          associations: rows.map((row: any) => ({
       ...
       * Disease -> targets
       */
      if (args.diseaseId && !args.targetId) {
        const query = `
          query GetDiseaseAssociations($efoId: String!, $pageIndex: Int!, $pageSize: Int!) {
            disease(efoId: $efoId) {
              id
              name
        ...
        // Fix: same local-const capture as above (TS18048).
        if (args.minScore !== undefined) {
          const minScore = args.minScore;
          rows = rows.filter((row: any) => row.score >= minScore);
        }
        ...
        return this.textResponse({
          ...
          totalAssociations: disease.associatedTargets?.count ?? 0,
          pageIndex,
          requestedSize: size,
          returned: rows.length,
          associations: rows.map((row: any) => ({
      ...
      return this.errorResponse('Error getting target-disease associations', error);
    }
  }
  ...
  private async handleGetDiseaseTargetsSummary(args: unknown) {
    ...
    try {
      ...
      const query = `
        query GetDiseaseTargetsSummary($efoId: String!, $pageIndex: Int!, $pageSize: Int!) {
          disease(efoId: $efoId) {
            id
            name
            associatedTargets(page: { index: $pageIndex, size: $pageSize }) {
       ...
       *
       * Fix: capture minScore in a local const before the filter
       * closure so TypeScript can prove it's defined (TS18048).
       */
      if (args.minScore !== undefined) {
        const minScore = args.minScore;
        rows = rows.filter((row: any) => row.score >= minScore);
      ...
      return this.errorResponse('Error getting disease targets summary', error);
    }
  }
  ...
  private async handleGetDiseaseDrugs(args: unknown) {
    if (!isValidDiseaseDrugsArgs(args)) {
      throw new McpError(ErrorCode.InvalidParams, 'Invalid disease drugs arguments');
    }

    try {
      const size = getSize(args.size, DEFAULT_DRUG_LIMIT, MAX_DRUG_LIMIT);
      const pageIndex = getPageIndex(args.pageIndex);
       ...
       */
      const query = `
        query GetDiseaseDrugs($efoId: String!, $pageIndex: Int!, $pageSize: Int!) {
          disease(efoId: $efoId) {
            id
            name
       ...
       */
      const allRows = disease.drugAndClinicalCandidates?.rows ?? [];
      ...
      return this.textResponse({
        ...
        totalCandidates: disease.drugAndClinicalCandidates?.count ?? 0,
        pageIndex,
        requestedSize: size,
        returned: rows.length,
        candidates: rows.map((row: any) => ({
          clinicalStage: row.maxClinicalStage ?? row.drug?.maximumClinicalStage ?? null,
          drug: row.drug
            ? {
                id: row.drug.id,
                name: row.drug.name,
                type: row.drug.drugType,
                maximumClinicalStage: row.drug.maximumClinicalStage,
              }
            : null,
        })),
      });
    } catch (error) {
      return this.errorResponse('Error getting disease drugs', error);
    }
  }
  ...
  private async handleGetTargetDrugs(args: unknown) {
    if (!isValidTargetDrugsArgs(args)) {
      throw new McpError(ErrorCode.InvalidParams, 'Invalid target drugs arguments');
    }
    ...
    try {
       ...
       */
      const query = `
        query GetTargetDrugs($ensemblId: String!, $pageIndex: Int!, $pageSize: Int!) {
          target(ensemblId: $ensemblId) {
            id
            approvedSymbol
            approvedName
      ...
      const allRows = target.knownDrugs?.rows ?? [];
      ...
      return this.textResponse({
        ...
        totalKnownDrugs: target.knownDrugs?.count ?? 0,
        pageIndex,
        requestedSize: size,
        returned: rows.length,
        drugs: rows.map((row: any) => ({
      ...
      return this.errorResponse('Error getting target drugs', error);
    }
  }
  ...
  private async fetchTargetDetails(targetId: string) {
    const query = `
  ...
  private async handleGetTargetDetails(args: unknown) {
    if (!isValidIdArgs(args)) {
      throw new McpError(ErrorCode.InvalidParams, 'Target ID is required');
    }

    try {
      const result = await this.fetchTargetDetails(args.id);
      return this.textResponse(result);
    } catch (error) {
      return this.errorResponse('Error getting target details', error);
    }
  }
  ...
  private async fetchDiseaseDetails(diseaseId: string) {
    const query = `
  ...
  private async handleGetDiseaseDetails(args: unknown) {
    if (!isValidIdArgs(args)) {
      throw new McpError(ErrorCode.InvalidParams, 'Disease ID is required');
    }

    try {
      const result = await this.fetchDiseaseDetails(args.id);
      return this.textResponse(result);
    } catch (error) {
      return this.errorResponse('Error getting disease details', error);
    }
  }
...
server.run().catch(error => {
  console.error('Failed to start Open Targets MCP server:', error);
  process.exit(1);
});
