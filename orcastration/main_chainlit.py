...
#    but noisy autogen spans are simply not exported to Arize.
class FilteringSpanProcessor(SpanProcessor):
    """
    Wraps a delegate SpanProcessor and only forwards spans whose names
    do NOT match the filter. Filtered spans are still recorded (so their
    context propagates to children), but never exported.
    """
    ...
    # during `initialize_agents()` — *before* any `run_stream` parent span
    # exists — so they always become orphan root traces in the backend.
    ...
    def on_end(self, span: ReadableSpan):
        ...
        # else: span is silently swallowed — parent-child context stays intact
...
# Initialize provider WITHOUT a custom sampler — all spans are sampled
# (AlwaysOn by default), so parent-child hierarchy is never broken.
provider = TracerProvider(
    space_id=ARIZE_SPACE_ID,
    api_key=ARIZE_API_KEY,
    resource=Resource(attributes={"model_id": "mini-pharmamind", "arize.project.name": "mini-pharmamind"})
)

# Wrap the BatchSpanProcessor with our filter so autogen spans are
