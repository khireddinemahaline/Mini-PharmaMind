...
# Optional MLflow integration (disabled in mini version by default)
# Uncomment to enable MLflow tracking when MLflow server is running
try:
    import mlflow
    mlflow_enabled = os.getenv("MLFLOW_ENABLED", "false").lower() == "true"
    if mlflow_enabled:
        mlflow.set_tracking_uri(os.getenv("MLFLOW_TRACKING_URI", "http://localhost:5000"))
        mlflow.set_experiment(os.getenv("MLFLOW_EXPERIMENT", "Mini-PharmaMind"))
...
async def save_team_state_to_disk(
    ...
    try:
        ...
        print(f"❌ File I/O error saving team state: {str(e)}")
        return None
    except Exception as e:
        print(f"❌ Unexpected error saving team state: {str(e)}")
        return None
...
async def load_team_state_from_disk(
    ...
    try:
        ...
        print(f"❌ File I/O error loading team state: {str(e)}")
        return False
    except (json.JSONDecodeError, ValueError) as e:
        print(f"❌ Invalid JSON in state file: {str(e)}")
        return False
    except Exception as e:
        print(f"❌ Unexpected error loading team state: {str(e)}")
        return False
